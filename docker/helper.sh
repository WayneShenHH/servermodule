...
SERVICE_NAME="svc-${SERVICE_NAME##*/}"      # remove everything before the last /
IMAGE_TAG=`git rev-parse --short=6 HEAD`        # retrive tag to specified git tag
IMAGE_PREFIX="images.wayne.shen.io/"   # this vary with different project name
...
function retrive_binary() {
    # 1.processing
    docker run -it --rm --name=$PROJECT_NAME --entrypoint=/bin/bash -d $IMAGE_NAME
    # 2.wait a while (1s)
    sleep 1
    # 3.retrive binary from container
    docker cp $PROJECT_NAME:/app .
    # 4.stop processing
